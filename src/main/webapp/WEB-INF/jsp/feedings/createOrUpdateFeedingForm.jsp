<%@ page session="false" trimDirectiveWhitespaces="true" %>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
<%@ taglib prefix="petclinic" tagdir="/WEB-INF/tags" %>

<petclinic:layout pageName="feedings">

    <jsp:attribute name="customScript">
        <script>
            $(function () {
                $("#start_date").datepicker({dateFormat: 'yy/mm/dd'});
            });
        </script>
    </jsp:attribute>
    <jsp:body>
    	<h2>
        	<c:if test="${feeding['new']}">New </c:if> Feeding
    	</h2>
    
    	<form:form modelAttribute="feeding" class="form-horizontal" id="add-owner-form">
    		<input type="hidden" name="id" value="${feeding.id}"/>
        	<div class="form-group has-feedback">
        
            	<petclinic:inputField label="Date" name="startDate"/>
            	<petclinic:inputField label="Weeks duration" name="weeksDuration"/>
            
            	<div class="control-group">
            		<petclinic:selectField name="pet" label="Pet" names="${pets}" size="2"/>
            	</div>
            	<div class="control-group">
            		<petclinic:selectField name="feedingType" label="Feeding Type" names="${feedingTypes}" size="2"/>
            	</div>  
        	</div>
        	<div class="form-group">
            	<div class="col-sm-offset-2 col-sm-10">
                	<c:choose>
                    	<c:when test="${feeding['new']}">
                        	<button class="btn btn-default" type="submit">Add Feeding</button>
                    	</c:when>
                    	<c:otherwise>
                        	<button class="btn btn-default" type="submit">Update Feeding</button>
                    	</c:otherwise>
                	</c:choose>
            	</div>
        	</div>
    	</form:form>
    	<c:if test="${!feeding['new']}">
    	</c:if>
    </jsp:body>
</petclinic:layout>       